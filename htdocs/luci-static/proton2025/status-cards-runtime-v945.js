...
/* PROTON_STATUS_CARDS_SAFE_RUNTIME_V945 */

var DISABLED_KEY = 'proton-status-cards-disabled-v944';
var BOX = 'proton-status-cards-v945';
var STYLE = 'proton-status-cards-v945-style';
var CHECK = 'proton-status-cards-toggle-v945';
var URL = '/luci-static/proton2025/status.json';
var LAST = 'proton-status-cards-v945-last';
var timer = null;
...
function isOverviewPath() {
  return String(location.pathname || '').indexOf('/admin/status/overview') >= 0;
}
...
function pageReady() {
  ...
  var t = String(main.innerText || '').replace(/\s+/g, ' ');

  if (t.indexOf('Загрузка страницы') >= 0 || t.indexOf('Loading view') >= 0)
    return false;

  return t.indexOf('Система') >= 0 || t.indexOf('System') >= 0 || t.indexOf('Uptime') >= 0 || t.indexOf('Время работы') >= 0;
}
...
function ensureBox() {
  if (!isOverviewPath() || !pageReady()) return null;

  var old = document.getElementById('proton-status-cards-v944') || document.getElementById('proton-status-cards-v943') || document.getElementById('proton-status-cards-v930');
  if (old) old.remove();
...
function fetchData() {
  if (!enabled() || !isOverviewPath() || !pageReady()) return;
...
function setting() {
  ...
  block.className = 'cbi-value proton-status-cards-toggle-setting-v945';
  block.innerHTML =
