...
/* PROTON_STATUS_CARDS_SPA_WATCH_V947 */

var BOX = 'proton-status-cards-v947';
var STYLE = 'proton-status-cards-v947-style';
var CHECK = 'proton-status-cards-toggle-v947';
var DISABLED_KEY = 'proton-status-cards-disabled-v947';
var URL = '/luci-static/proton2025/status.json';
var LAST = 'proton-status-cards-v947-last';
var lastHref = '';
var dataTimer = null;
...
function mainReady() {
  ...
  if (!txt) return false;
  if (txt.indexOf('Загрузка страницы') >= 0) return false;
  if (txt.indexOf('Loading view') >= 0) return false;
  if (txt.indexOf('Please wait') >= 0) return false;

  return true;
}

function overviewReady() {
  return isOverviewUrl() && mainReady();
}
...
function fetchData() {
  if (!enabled() || !overviewReady()) return;
...
function settings() {
  ...
  block.className = 'cbi-value proton-status-cards-toggle-setting-v947';
  block.innerHTML =
...
function boot() {
  ...
  if (lastHref !== String(location.href || '')) {
...
if (document.readyState === 'loading')
  document.addEventListener('DOMContentLoaded', safeBootSoon);
else
  safeBootSoon();

window.addEventListener('load', safeBootSoon);
window.addEventListener('popstate', safeBootSoon);
window.addEventListener('hashchange', safeBootSoon);
window.addEventListener('storage', boot);
document.addEventListener('click', function () {
  safeBootSoon();
}, true);

/* LuCI SPA: footer не перезагружается при переходах, поэтому нужен постоянный лёгкий watcher */
