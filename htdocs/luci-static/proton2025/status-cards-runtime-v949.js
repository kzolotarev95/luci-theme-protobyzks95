...
/* PROTON_STATUS_CARDS_CONTENT_DETECT_V949 */

var BOX = 'proton-status-cards-v949';
var STYLE = 'proton-status-cards-v949-style';
var CHECK = 'proton-status-cards-toggle-v949';
var DISABLED_KEY = 'proton-status-cards-disabled-v949';
var URL = '/luci-static/proton2025/status.json';
var LAST = 'proton-status-cards-v949-last';
...
function text() {
  var m = main();
  return m ? String(m.innerText || '').replace(/\s+/g, ' ') : '';
}

function loading() {
  var t = text();
  return !t || t.indexOf('Загрузка страницы') >= 0 || t.indexOf('Loading view') >= 0 || t.indexOf('Please wait') >= 0;
}

function looksLikeStatusOverview() {
  if (loading()) return false;

  var u = String(location.href || '');
  if (u.indexOf('/admin/status/overview') >= 0 || u.indexOf('status/overview') >= 0)
    return true;

  var t = text();
  var score = 0;

  if (/Имя хоста|Hostname/i.test(t)) score++;
  if (/Модель|Model/i.test(t)) score++;
  if (/Архитектура|Architecture/i.test(t)) score++;
  if (/Версия прошивки|Firmware Version/i.test(t)) score++;
  if (/Время работы|Uptime/i.test(t)) score++;
  if (/Нагрузка|Load Average|Load average/i.test(t)) score++;
  if (/Память|Memory/i.test(t)) score++;
  if (/Локальное время|Local Time|Системное время/i.test(t)) score++;
...
function removeOldBoxes() {
  [
    ...
    'proton-status-cards-v946',
    'proton-status-cards-v947',
    'proton-status-cards-v948'
  ].forEach(function (id) {
...
function ensureBox() {
  if (!looksLikeStatusOverview()) return null;
...
function fetchData() {
  if (!enabled() || !looksLikeStatusOverview()) return;
...
function settings() {
  ...
  block.className = 'cbi-value proton-status-cards-toggle-setting-v949';
  block.innerHTML =
...
function boot() {
  ...
  if (!enabled()) {
    var b0 = document.getElementById(BOX);
