...
/* PROTON_STATUS_CARDS_LOGIN_STATUS_V948 */

var BOX = 'proton-status-cards-v948';
var STYLE = 'proton-status-cards-v948-style';
var CHECK = 'proton-status-cards-toggle-v948';
var DISABLED_KEY = 'proton-status-cards-disabled-v948';
var URL = '/luci-static/proton2025/status.json';
var LAST = 'proton-status-cards-v948-last';
var lastUrl = '';
...
function loading() {
  var m = main();
  var t = m ? String(m.innerText || '').replace(/\s+/g, ' ') : '';
  return !m || t.indexOf('Загрузка страницы') >= 0 || t.indexOf('Loading view') >= 0 || t.indexOf('Please wait') >= 0;
}
...
function statusPage() {
  ...
  var h = '';
  var hs = m.querySelectorAll('h1,h2,h3');
  for (var i = 0; i < hs.length; i++)
    h += ' ' + String(hs[i].innerText || '');
  ...
  var active = document.querySelector('.mainmenu .active, .tabs .active, .cbi-tab, nav .active');
  var at = active ? String(active.innerText || '') : '';

  if (/Статус|Status/i.test(at) && /Статус|Status/i.test(h))
    return true;
...
function settings() {
  ...
  block.className = 'cbi-value proton-status-cards-toggle-setting-v948';
  block.innerHTML =
...
function boot() {
  ...
  if (lastUrl !== String(location.href || '')) {
...
/* лёгкий постоянный watcher для первого экрана после логина */
